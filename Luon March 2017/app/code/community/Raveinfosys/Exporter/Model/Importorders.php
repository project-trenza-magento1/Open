...
class Raveinfosys_Exporter_Model_Importorders extends Mage_Core_Model_Abstract
{
    ...
    public function readCSV($csvFile, $data)
    {
        try{
            ...
            #mail('faroque.golam@gmail.com' , 'test 11' , 'before read file');
            
            while (!feof($file_handle)) {
                $line_of_text[] = fgetcsv($file_handle);
                
                
                #mail('faroque.golam@gmail.com' , 'order track ' . $i , 'order line number');
                ...
                if ($i != 0) {
                    if ($line_of_text[$i][0] != '' && $parent_flag == 0) {
                        ...
                        try {
                            ...
                            mail('faroque.golam@gmail.com' , 'error track ' . $i , $e->getMessage());
                            Mage::getSingleton('adminhtml/session')->addError($e->getMessage() . ' >>> ' . $this->order_info['increment_id'] . ' >>> ' .  'order' . ' >>> ' .  $line_number);
