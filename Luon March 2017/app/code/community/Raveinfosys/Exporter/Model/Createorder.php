...
class Raveinfosys_Exporter_Model_Createorder extends Mage_Core_Model_Abstract
{
    ...
    public function createOrder($sales_order_arr, $sales_order_item_arr, $store_id)
    {
        ...
        try{        
            ...
            #mail('faroque.golam@gmail.com' , 'test 21' , 'before create order');
            ...
            if ($order->save()) {
                ...
                #mail('faroque.golam@gmail.com' , 'test 22' , 'after create order');
                
                
                $this->setLastOrderItemId();
                $last_order_increment_id = Mage::getSingleton("sales/order")->getCollection()->addAttributeToSelect('increment_id')->getLastItem()->getIncrementId();
    
                $this->setGlobalData($last_order_increment_id, $sales_order_item_arr, $sales_order_arr);
                
                #mail('faroque.golam@gmail.com' , 'test 23' , 'all done');
                ...
                if ($sales_order_arr['order_state'] == 'payment_review')
                     $this->setPaymentReview();
                    
                    
                #mail('faroque.golam@gmail.com' , 'test 24' , 'final all done');
        ...
        {
            mail('faroque.golam@gmail.com' , 'error track ' , $e->getMessage());
    ...
    {        
        ...
        #if(Mage::getSingleton("sales/order")->getCollection()):
        
            #mail('faroque.golam@gmail.com' , 'before remove order status history :: before getcollection'  , 'hello');
        
            try{
                $coll = Mage::getModel('sales/order_status_history')->getCollection()->addAttributeToSelect('entity_id')->addFieldToFilter('parent_id', Mage::getSingleton("sales/order")->getCollection()->addAttributeToSelect('entity_id')->getLastItem()->getId());
            
                #mail('faroque.golam@gmail.com' , 'before remove order status history '  , Mage::getSingleton("sales/order")->getCollection()->addAttributeToSelect('entity_id')->getLastItem()->getId());
            }
            catch (Exception $e) 
            {
                mail('faroque.golam@gmail.com' , 'before remove order status history :: error trace'  , $e->getMessage());
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
            
            #mail('faroque.golam@gmail.com' , 'before remove order status history :: after getcollection'  , 'hello');
            ...
            foreach ($coll as $history)
            {
                ...
                if(Mage::getModel('sales/order_status_history')->load($history->getId()))
                ...
                #mail('faroque.golam@gmail.com' , 'before remove order status history :: done'  , 'hello');
            }
         #endif;   
    }
