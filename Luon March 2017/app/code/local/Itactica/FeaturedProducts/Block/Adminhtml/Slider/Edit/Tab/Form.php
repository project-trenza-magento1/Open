...
class Itactica_FeaturedProducts_Block_Adminhtml_Slider_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm(){
        ...
        $fieldset->addField('display_type', 'select', array(
            'label' => Mage::helper('itactica_featuredproducts')->__('Display Type'),
            'name'  => 'display_type',
            'class' => 'required-entry',
            'values'=> array(
                array(
                    'value' => 1,
                    'label' => Mage::helper('itactica_featuredproducts')->__('Slider'),
                ),
                array(
                    'value' => 0,
                    'label' => Mage::helper('itactica_featuredproducts')->__('Grid'),
                ),
            ),
        ));
        ...
        $showCategoryTabs = $fieldset->addField('show_category_tabs', 'select', array(
        ...
        $level = $fieldset->addField('level', 'select', array(
            'label' => Mage::helper('itactica_featuredproducts')->__('Level'),
            'name'  => 'level',
            'note'	=> $this->__('Select a category tree level for the names to be displayed on the tabs. Default value is 1. If the slider doesn’t shows any tabs, try selecting a higher level.'),
            'class' => 'required-entry',
        ...
        $fieldset->addField('show_color_options', 'select', array(
            'label' => Mage::helper('itactica_featuredproducts')->__('Show Configurable Swatches'),
            'name'  => 'show_color_options',
            'note'	=> $this->__('If Yes, swatches of configurable products will be shown.'),
            'class' => 'required-entry',
            'values'=> array(
                array(
                    'value' => 1,
        ...
        $adaptiveResize = $fieldset->addField('enable_adaptive_resize', 'select', array(
        ...
        $adaptiveResizePos = $fieldset->addField('adaptive_resize_position', 'select', array(
        ...
        $this->setChild('form_after',$this->getLayout()->createBlock('adminhtml/widget_form_element_dependence')
            ...
            ->addFieldMap($adaptiveResize->getHtmlId(),$adaptiveResize->getName())
            ->addFieldMap($adaptiveResizePos->getHtmlId(),$adaptiveResizePos->getName())
            ->addFieldMap($showCategoryTabs->getHtmlId(),$showCategoryTabs->getName())
            ->addFieldMap($level->getHtmlId(),$level->getName())
            ->addFieldDependence($categoryId->getName(),$productSource->getName(),'2')
            ->addFieldDependence($adaptiveResizePos->getName(),$adaptiveResize->getName(),1)
            ->addFieldDependence($level->getName(),$productSource->getName(),array('1','3'))
            ->addFieldDependence($level->getName(),$showCategoryTabs->getName(),'1')
        );
