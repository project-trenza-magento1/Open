...
class Itactica_FeaturedProducts_Block_View extends Mage_Catalog_Block_Product_Abstract 
{
    ...
    public function getCategories()
    {
        ...
        if ($productsSource == 2) {
            if ($this->_slider->getShowCategoryTabs()) {
                $subcategoryCollection = Mage::getModel('catalog/category')
                    ...
                    ->addAttributeToFilter('entity_id', array('in' => $selectedCategories))
        ...
        if ($this->_slider->getShowCategoryTabs()) {
            ...
            if ($productsSource == 2) {
                // removes categories not selected in category tree field
                $sortHelper = array_intersect($sortHelper, $selectedCategories);
                $sortHelper = array_flip($sortHelper);
                $categoryNames = array_intersect_key($categoryTabs, $sortHelper);
            } else {
    ...
    public function getProductCollection($categoryId = 0, $applyLimit = true, $setImageFallback = false) 
    {
        ...
        switch ($sort) {
            ...
            case 6:
                $collection = $this->_getProductCollection($categoryId, 'new', $applyLimit, $setImageFallback);
                return $collection;
                break;
    ...
    public function _getProductCollection($categoryId, $sortBy = 'recent', $applyLimit, $setImageFallback)
    {
        ...
        if ($sortBy == 'rating') {
            ...
            $bestsellers = Mage::getSingleton('core/resource')->getConnection('core_read')
                ->select()
                ->from(Mage::getConfig()->getTablePrefix().'sales_flat_order_item', array('product_id', 'count' => 'SUM(`qty_ordered`)'))
                ->group('product_id');
