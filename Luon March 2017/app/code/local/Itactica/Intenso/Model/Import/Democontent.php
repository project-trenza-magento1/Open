	...
	private function createProduct($node, $storeId) {
		...
		if ($product->getId()) {
			...
			try {
				...
				if ($node->news_from_date != '') $newProduct->setNewsFromDate(strtotime($node->news_from_date));
				if ($node->news_to_date != '') $newProduct->setNewsToDate(strtotime($node->news_to_date));
				if ($node->special_from_date != '') $newProduct->setSpecialFromDate(strtotime($node->special_from_date));
				if ($node->special_to_date != '') $newProduct->setSpecialToDate(strtotime($node->special_to_date));
			...
			if (!is_readable($file)) {
				...
				foreach ($xmlObj->getNode('democontent')->children() as $node) {
					...
					$cmsPageData = array(
						...
						'layout_update_xml' => $node->layout_update_xml,
