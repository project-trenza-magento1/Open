...
class Amasty_Segments_Model_Segment_Condition_Order_Combine_Placed
    extends Amasty_Segments_Model_Condition_Abstract
{
    ...
    {
        parent::__construct();
        $this->setType('amsegments/segment_condition_order_combine_placed');
        $this->setValue(null);
    }
...
//        }
    ...
    public function index($websiteIds, $combineCondition)
    {
        ...
        $select->where("salesOrder.created_at " . $operator . " ? ", $value);
...
//            $value = $this->getValue();
//            
//            return $adapter->quoteInto("salesOrder.status " . $operator . " ? ", $value);
//        }
}
