...
class Amasty_Segments_Model_Segment_Condition_Cart_Daysfromcreated
    extends Amasty_Segments_Model_Condition_Abstract
{
    ...
    {
        parent::__construct();
        $this->setType('amsegments/segment_condition_cart_daysfromcreated');
        $this->setValue(null);
    }
   ...
   public function asHtml()
   {
    ...
    protected function _getResultExpr()
    {
       ...
       $sql = $this->getResource()->getReadConnection()->quoteInto("FIND_IN_SET(1, GROUP_CONCAT(DATEDIFF(NOW(), salesQuote.created_at)  {$operator} ?)) <> 0", $value);
