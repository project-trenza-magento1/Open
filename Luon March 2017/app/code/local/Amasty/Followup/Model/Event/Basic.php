...
class Amasty_Followup_Model_Event_Basic extends Varien_Object
{
    ...
    protected function _validateCustomer($customerEmail)
    {
        ...
        if (Mage::getConfig()->getNode('modules/Amasty_Segments/active') == 'true' && !empty($segments)) {
    ...
    {
        return $this->_rule->getIsActive() == 1 && $this->_validateStore($storeId)
    ...
    static public function getLastExecuted()
    {
    ...
    static public function getCurrentExecution()
    {
        return self::$_currentExecution ? self::$_currentExecution : time();
    }

    public function getEmail($schedule, $history, $vars = array())
    {
        ...
        if (!$emailTemplate->getId()) {
            throw Mage::exception(
                'Mage_Core',
                Mage::helper('core')->__('Invalid transactional email code: ' . $templateId)
            );
        }
