...
class Amasty_Followup_Model_Event_Order_Status extends Amasty_Followup_Model_Event_Basic
{
    ...
    protected function _initCollection()
    {
        ...
        $collection->getSelect()->joinInner(
            ...
            array('order_history.created_at as target_created_at', 'order_history.entity_id as entity_id_history')
        );

        $collection->addFieldToFilter(
            'order_history.created_at',
            array('gteq' => $this->date($this->getLastExecuted()))
        );

        $collection->addFieldToFilter(
            'order_history.created_at',
            array('lt' => $this->date($this->getCurrentExecution()))
        );
        ...
        if (version_compare(Mage::getVersion(), '1.5', '>')) {
            ...
            if ($historyEntityName) {
                $collection->addFieldToFilter(
                    'order_history.entity_name',
                    array('eq' => $historyEntityName)
                );
            }
        }
