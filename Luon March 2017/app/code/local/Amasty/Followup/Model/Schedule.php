...
class Amasty_Followup_Model_Schedule extends Mage_Core_Model_Abstract
{
    ...
    {
        if ($this->getDelayedStart() > 0) {
            return floor($this->getDelayedStart() / 24 / 60 / 60);
        }
        return  null;
    }
    ...
    protected function _loadCustomerLog($customer)
    {
        if (!isset($this->_customerLog[$customer->getId()])) {
            ...
            $this->_customerLog[$customer->getId()]->setInactiveDays(
                floor(
                    (time() - strtotime($this->_customerLog[$customer->getId()]->getLoginAt())) / 60 / 60 / 24
                )
            );
    ...
    public function getOrderEmailVars($order, $quote, $customer, $history)
    {
        return $this->_getOrderEmailVars($order, $quote, $customer, $history);
    }

    public function checkCustomerRules($customer, $types = array())
    {
    ...
    public function createOrderHistory($rule, $event, $order, $quote, $customer)
    {
        ...
        foreach ($scheduleCollection as $schedule) {
            $historyCollection = Mage::getResourceModel('amfollowup/history_collection')
                ->addFieldToFilter('order_id', $order->getId())
                ->addFieldToFilter('rule_id', $schedule->getRuleId())
                ->addFieldToFilter('status', array('neq' => Amasty_Followup_Model_History::STATUS_CANCEL));

            /** create only one item for current order by current rule */
            if ($historyCollection->getSize()) {
                return $ret;
            }

            Mage::app()->setCurrentStore($quote->getStoreId());
            /** @var Amasty_Followup_Model_History $history */
            $history = Mage::getModel("amfollowup/history");
