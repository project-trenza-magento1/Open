...
class AW_Advancedreports_Block_Advanced_Grid extends Mage_Adminhtml_Block_Report_Grid
{
    ...
    public function _getMysqlFromFormat($date)
    {
        ...
        $dateFrom = Mage::app()->getLocale()
            ->utcDate(null, $dateFrom->toString(Varien_Date::DATETIME_INTERNAL_FORMAT), true, Varien_Date::DATETIME_INTERNAL_FORMAT)
        ;
        return $dateFrom->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
    }
    ...
    public function _getMysqlToFormat($date)
    {
        ...
        $dateTo = Mage::app()->getLocale()
            ->utcDate(null, $dateTo->toString(Varien_Date::DATETIME_INTERNAL_FORMAT), true, Varien_Date::DATETIME_INTERNAL_FORMAT)
        ;
        return $dateTo->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
    }
    ...
    public function getStoreIds()
    {
        if (!$this->_storeIds) {
            ...
            if (Mage::helper('advancedreports')->isModuleEnabled('Enterprise_AdminGws')) {
                 $_restrictedRoles = Mage::getSingleton('enterprise_admingws/role');
                if (!$_restrictedRoles->getIsAll()) {
                    $_restrictedStores = $_restrictedRoles->getStoreIds();
                    if (count($storeIds) > 0) {
                        $_allowedStores = array_intersect($_restrictedStores, $storeIds);
                        if (count($_allowedStores) > 0) {
                            $_restrictedStores = $_allowedStores;
                        }
                    }
                    $storeIds = $_restrictedStores;
                }
            }
    ...
    protected function _prepareFiltersBefore()
    {
        ...
        if (!isset($data['report_from'])) {
            if ($this->getFilter('report_from')) {
                ...
                $date = new Zend_Date(null, null, Mage_Core_Model_Locale::DEFAULT_LOCALE);
                $date->setDay(1);
