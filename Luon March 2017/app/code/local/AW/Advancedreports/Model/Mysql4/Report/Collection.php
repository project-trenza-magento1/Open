...
class AW_Advancedreports_Model_Mysql4_Report_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    protected function processIntervals()
    {
        foreach ($this->_intervals as &$interval) {
            $interval['start'] = Mage::app()->getLocale()
                ->utcDate(null, $interval['start'], true, Varien_Date::DATETIME_INTERNAL_FORMAT)
                ->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);

            $interval['end'] = Mage::app()->getLocale()
                ->utcDate(null, $interval['end'], true, Varien_Date::DATETIME_INTERNAL_FORMAT)
                ->toString(Varien_Date::DATETIME_INTERNAL_FORMAT);
        }
    }
